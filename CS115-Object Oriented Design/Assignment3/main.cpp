...
int main()
{
  ...
  while (playing)
  {
    ...
    else if (isPlaceStringWellFormed(move_string))
    {
      ...
      {
        if (board.getAt(row, column) == BOARD_VALUE_EMPTY)
        {
          cout << "Black played a stone at row " << row << ", column " << column
               << endl;
          board.setAt(row, column, BOARD_VALUE_BLACK);
          ...
          cout << "Forbidden: Place row " << row << ", column " << column
               << " is not empty" << endl;
      }
      else
        cout << "Forbidden: Place row " << row << ", column " << column
...
void printEnd(const Board &board_in, const string moves_in[], int move_count_in)
{
  float black_score = board_in.countWithValue( BOARD_VALUE_BLACK) + 0.0f;
  float white_score = board_in.countWithValue( BOARD_VALUE_WHITE) + KOMI;
  ...
  if (move_count_in > 0)
  {
    ...
    cout << "Smallest place was \"" << moves_in[smallest] << "\" (index "
         << smallest << ")" << endl;
    cout << "Largest place was \"" << moves_in[largest] << "\" (index "
         << largest << ")" << endl;
